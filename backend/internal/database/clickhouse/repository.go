...
func NewRepository(manager *Manager) database.Repository {
	r := &repositoryImpl{
		mngr:    manager,
		queue:   make(chan database.Query, batchSize*2), // buffer avoids blocking callers
...
func (r *repositoryImpl) SaveQuery(_ context.Context, q database.Query) error {
	select {
	case r.queue <- q:
		return nil
	case <-r.stopped:
...
func (r *repositoryImpl) batchWorker() {
	...
	batch := make([]database.Query, 0, batchSize)
...
func (r *repositoryImpl) flushBatch(queries []database.Query) error {
	ctx := context.Background()

	b, err := r.mngr.conn.PrepareBatch(ctx, `
...
func (r *repositoryImpl) FindAllLimit(ctx context.Context, limit int, name string) ([]database.Query, error) {
  ...
  `
	...
	rows, err := r.mngr.conn.Query(ctx, baseQuery, args...)
	if err != nil {
...
func (r *repositoryImpl) FindAllByInterval(ctx context.Context, since time.Time) ([]database.Query, error) {
	...
	rows, err := r.mngr.conn.Query(ctx, q, since)
...
func (r *repositoryImpl) FindDomainDetailsPoints(ctx context.Context, name string, since time.Time, granularity time.Duration) ([]database.Point, error) {
	...
	rows, err := r.mngr.conn.Query(ctx, q, name, since)
	if err != nil {
